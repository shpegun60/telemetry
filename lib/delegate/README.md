...
Telemetry's compact Getter/Setter implementation no longer depends on this
header, so applications include `delegate.pri` only when they use tiny_delegate
directly. No Qt dependency, separate compilation or machine-specific path is
required.
