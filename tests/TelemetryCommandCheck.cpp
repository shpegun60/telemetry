...
constexpr CommandIndex index{commands};
static_assert(index.size()==7 && commands[0].metadata==nullptr);
...
int main()
{
    expect(index.call(0)==CommandResult::Executed && device.calls==1,"zero-argument method");
    expect(index.call(1,250,Mode::Precise)==CommandResult::Executed && device.voltage==250.0f && device.mode==Mode::Precise,"typed member conversion");
    const auto before=device.calls;
    expect(index.call(1,250)==CommandResult::ArgumentCountMismatch && device.calls==before,"missing argument has no side effect");
    expect(index.call(1,250,1,7)==CommandResult::ArgumentCountMismatch,"excess arguments");
    expect(index.call(1,501.0f,1)==CommandResult::InvalidValue && device.calls==before,"custom limit");
    expect(index.call(1,250,99)==CommandResult::InvalidValue && device.calls==before,"last invalid arg prevents whole command");
    expect(index.call(1,Scalar{},1)==CommandResult::InvalidValue,"null arg rejected");
    expect(index.execute(1,nullptr,2)==CommandResult::InvalidValue,"null pointer with matching count");
    expect(index.execute(1,nullptr,0)==CommandResult::ArgumentCountMismatch,"count checked first");
    expect(index.call(100)==CommandResult::NotFound,"command ID out of bounds");
    expect(Command{}.call()==CommandResult::Unavailable,"empty command safe");
    expect(index.call(2,UINT64_MAX)==CommandResult::Accepted && device.address==UINT64_MAX,"U64 exact and Accepted forwarded");
    expect(index.call(2,-1)==CommandResult::InvalidValue,"negative to U64 rejected");
    expect(index.call(2,0)==CommandResult::InvalidValue,"U64 lower bound");
    expect(index.call(3,INT64_MIN,true)==CommandResult::Failed && freeCalls==1,"free function return preserved");
    expect(index.call(4)==CommandResult::Executed,"static method without owner");
    ...
    expect(index.call(5,-50.0,2)==CommandResult::Executed && device.voltage==-50.0f,"inferred native bounds");
    expect(index.call(5,std::numeric_limits<float>::infinity(),1)==CommandResult::InvalidValue,"infinite native input rejected");
    expect(index.call(5,std::numeric_limits<float>::quiet_NaN(),1)==CommandResult::InvalidValue,"nan native input rejected");
    expect(index.call(6,0)==CommandResult::Executed,"unfixed enum gap allowed");
    expect(index.call(6,1000)==CommandResult::InvalidValue,"unfixed enum cast guarded");
    Device local;
    ...
    const auto n=writeSchema(index,json.data(),json.size());
    ...
    expect(writeSchema(index,strings.data(),strings.size(),{JsonInt64Mode::String})!=0
    ...
    expect(boundaries(index,{}) && boundaries(index,{JsonInt64Mode::String}),"all schema buffer boundaries");
    expect(writeSchema(index,nullptr,0)==0 && writeSchema(index,nullptr,4096)==0,"null buffers");
    ...
    expect(schemaCrc(CommandIndex{})!=schemaCrc(index),"nonempty fingerprint");
